...
IMPLEMENT_SERIAL(TrapezoidF, CObject, 1)

TrapezoidF::TrapezoidF(CPoint p1, CPoint p2)
	: RectangleF(p1, p2) {
	kind = 6;
}

TrapezoidF::TrapezoidF(CPoint p1, CPoint p2, int ID)
	: RectangleF(p1, p2, ID) {
	kind = 6;
}

void TrapezoidF::Draw(CDC* dc) const {
	CBrush cb;
...
bool TrapezoidF::isInside(const CPoint& P) const {
	...
	CPoint A, B, C, D;
	A.x = getP1().x + abs(getP2().x - getP1().x) / 4;
	A.y = getP1().y;
	...
	double BA = sqrt((A.x - B.x) * (A.x - B.x) + (A.y - B.y) * (A.y - B.y)); // base 1
	double DC = sqrt((D.x - C.x) * (D.x - C.x) + (D.y - C.y) * (D.y - C.y)); // base 2
	...
	// S(ABCD)
	double ttlSpace = ((BA + DC) / 2) * abs(getP2().y - getP1().y);
	...
	// S(PDC)
	d = (PD + PC + DC) / 2;
	double s4 = sqrt(d * (d - PD) * (d - PC) * (d - DC));
